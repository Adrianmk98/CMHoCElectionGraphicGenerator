...
def lighten_color(color_name, factor):
    ...
    # Get the hex value for the color name
    hex_color = color_map.get(color_name, '#d3d3d3')  # Default to gray if no match
    ...
    # Return the lighter color as hex
...
def update_svg_fill(input_svg, output_svg,output_json, riding_results, year_index, ratios):
    ...
    print(f"TOKYO Riding Results: {riding_results}")
    tree = ET.parse(input_svg)
    root = tree.getroot()
    color_map = {party['short_pname']: party['color'] for party in all_parties}
    parties = [party['short_pname'] for party in all_parties]
    ...
    for elem in root.findall(".//*[@data-riding]"):
        ...
        if elem_riding_name in riding_results:
            ...
            for party_index, party in enumerate(parties):
                print("GENEVA",party_index, party)
...
def mapmaker_main(file_path, input_svg, output_dir,party_names, pop_votes,riding_name):
    ...
    print("BALTIMORE",riding_results)
